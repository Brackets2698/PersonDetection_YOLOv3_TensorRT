...
class PostprocessYOLO(object):
    ...
    def _process_yolo_output(self, outputs_reshaped, resolution_raw):
        ...
        for output, mask in zip(outputs_reshaped, self.masks):
            ...
            box, category, confidence = self._process_feats(output, mask) #Intensive function here
    ...
    def _process_feats(self, output_reshaped, mask):
        ...
        box_xy = F.sigmoid(output_reshaped[..., :2])
        
        box_wh = F.exp(output_reshaped[..., 2:4]) * anchors_tensor
        box_confidence = F.sigmoid(output_reshaped[..., 4])

        box_confidence = F.expand_dims(box_confidence, axis=-1)
        box_class_probs = F.sigmoid(output_reshaped[..., 5:])
        ...
        box_xy += grid
        box_xy /= np.array([grid_w, grid_h])
        box_wh /= np.array(self.input_resolution_yolo)
        box_xy -= (box_wh / 2.)
        boxes = np.concatenate((box_xy.array, box_wh.array), axis=-1)

        # boxes: centroids, box_confidence: confidence level, box_class_probs:
        # class confidence
        return boxes, box_confidence.array, box_class_probs.array
